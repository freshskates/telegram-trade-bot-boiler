...
export async function fetchPumpTokenPrice(
  tokenAddress: string
): Promise<number> {
...
export async function fetchTokenDetails(tokenAddress: string) {
  ...
  try {
    ...
    if (
      response.data &&
      response.data.trc20_tokens &&
      response.data.trc20_tokens.length > 0
    ) {
      ...
      const totalSupply =
        parseFloat(tokenData.total_supply_with_decimals) / 1e18;
