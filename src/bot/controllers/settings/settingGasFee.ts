...
export const setGas = async (conversation: BotConversation, ctx: any) => {
    ...
    if (callbackData === "cb_settings_swapTokenToCoin_gas_fee_VALUE_1") {
        gasSetting = "Economy 🐴";
        selectedGasFee = 50;
    } else if (callbackData === "cb_settings_swapTokenToCoin_gas_fee_VALUE_2") {
        gasSetting = "Normal 🚀";
        selectedGasFee = 100;
    } else if (callbackData === "cb_settings_swapTokenToCoin_gas_fee_VALUE_3") {
        gasSetting = "Ultra 🦄";
        selectedGasFee = 200;
    } else if (callbackData === "cb_settings_swapTokenToCoin_gas_fee_VALUE_custom") {
...
bot.callbackQuery("cb_settings_swapTokenToCoin_gas_fee_VALUE_1", async (ctx) => {
    await ctx.conversation.enter("conversation_gasSetting");
    await ctx.answerCallbackQuery();
});

bot.callbackQuery("cb_settings_swapTokenToCoin_gas_fee_VALUE_2", async (ctx) => {
    await ctx.conversation.enter("conversation_gasSetting");
    await ctx.answerCallbackQuery();
});

bot.callbackQuery("cb_settings_swapTokenToCoin_gas_fee_VALUE_3", async (ctx) => {
    await ctx.conversation.enter("conversation_gasSetting");
    await ctx.answerCallbackQuery();
});

bot.callbackQuery("cb_settings_swapTokenToCoin_gas_fee_VALUE_custom", async (ctx) => {
    await ctx.conversation.enter("conversation_gasSetting");
    await ctx.answerCallbackQuery();
});
