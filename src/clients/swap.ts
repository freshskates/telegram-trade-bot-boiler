...
export class SwapClient {
  ...
  async swap(
    ...
    try {
      ...
      if (!isTRXFromToken) {
        const fromTokenDecimals = await getTokenDecimals(
          this.tronWeb,
          fromToken
        );
        amountInSun = BigInt(
      ...
      if (isTRXFromToken && !tokenPrice && routeInfo) {
        await approveToken(
          ...
          toToken,
          (routeInfo.amountOut * 1e18 * 4).toString()
        );
      } else if (isTRXFromToken) {
        await approveToken(
          this.tronWeb,
          this.pumpRouterAddress,
          toToken,
          (routeInfo.amountOut * 1e18 * 4).toString()
        );
      }
      await this.handleFeeTransaction(
        isTRXFromToken,
        isTRXToToken,
        amountInSun,
        routeInfo
  ...
  private async handleFeeTransaction(
    ...
    try {
      ...
      if (isTRXFromToken) {
        ...
        feeAmountSun =
          BigInt(Math.floor(routeInfo.amountOut * 1e6)) / BigInt(100);
      }
