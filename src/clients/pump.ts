...
import { approveToken, fetchPumpTokenPrice } from "../utils/helpers";
...
export class PumpClient {
  ...
  async pumpPurchase(
    ...
    try {
      ...
      const slippageMultiplier = BigInt(
        Math.floor((1 - slippage / 100) * 1e18)
      );
      ...
      await approveToken(
        ...
        this.proxyContractAddress,
        token,
        (expectedTokens * BigInt(10)).toString()
      );
  ...
  async pumpSellToken(
    ...
    try {
      const amountInWei = new BigNumber(tokenAmount)
      ...
      const expectedTRX = new BigNumber(tokenPrice)
        .multipliedBy(new BigNumber(tokenAmount))
        .multipliedBy(new BigNumber(1e6))
        .toFixed();

      // Apply slippage to calculate the minimum acceptable TRX amount
      const minTRX = new BigNumber(expectedTRX)
        .multipliedBy(new BigNumber(1 - slippage / 100))
        .toFixed(0);
